 ...
 */
const GlassTable = ({ columns, rows, rowKey, minWidth = "820px" }) => (
          ...
          <tr className="border-b border-white/10">
            {columns.map((c) => (
              <th
                key={c.header}
                scope="col"
                className={`px-4 py-3.5 text-left text-[10px] sm:text-[11px] font-bold uppercase tracking-[0.16em] text-gray-500 whitespace-nowrap ${c.thClassName || ""}`}
              >
                {c.header}
              </th>
            ))}
          ...
          {rows.map((row, i) => (
            ...
            >
              {columns.map((c) => (
                <td
                  key={c.header}
                  className={`px-4 py-3.5 text-sm align-top ${c.tdClassName || ""}`}
                >
                  {c.cell(row)}
                </td>
              ))}

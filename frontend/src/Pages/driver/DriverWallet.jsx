...
const DriverWallet = () => {
  ...
  return (
    <Motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      {walletError || txError ? (
        <ErrorState message={walletErr?.message || txErr?.message || 'Failed to load wallet'} onRetry={() => { queryClient.invalidateQueries({ queryKey: ['driverWallet'] }); queryClient.invalidateQueries({ queryKey: ['walletHistory'] }); }} />
      ) : (
        <>

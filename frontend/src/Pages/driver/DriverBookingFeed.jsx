...
const DriverBookingFeed = ({
  ...
  const { data, isLoading, isFetching, isError, error, refetch } = useQuery({
    ...
    queryFn: async () => {
      if (mode === "mine") {
        const { data } = await bookingAPI.getMyDriverBookings({
          page: 1,
          limit: 50,
        });
        return data;
  ...
  const hasActiveRide =
    gateOnActiveRide &&
    !!activeRide &&
    !["Completed", "Cancelled"].includes(activeRide.bookingStatus);
  ...
  const canAct = (b) =>
    mode !== "mine" && !b.driver && b.bookingStatus === "Pending";
  ...
  const handleAccept = (id) => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      () => {
        setAcceptingId(null);
        toast.error("Enable location to accept — opening details.", {
          duration: 4000,
        });
        openBooking(id);
      },
  ...
  const stats = useMemo(() => {
    ...
    const dist = allBookings
      .filter((b) => b.distance != null)
      .map((b) => Number(b.distance));
  ...
  const bookings = useMemo(() => {
    const list =
      payTab === "all"
        ? [...allBookings]
        : allBookings.filter((b) => b.paymentMethod === payTab);
    if (sort === "fare")
      list.sort(
        (a, b) =>
          (Number(b.estimatedFare) || 0) - (Number(a.estimatedFare) || 0),
      );
      ...
      list.sort(
        (a, b) =>
          new Date(b.createdAt || b.pickupDateTime) -
          new Date(a.createdAt || a.pickupDateTime),
      );
  ...
  const bookingColumns = [
    ...
    },
    {
      ...
      cell: (b) => (
        <div className="min-w-[180px] max-w-[260px]">
          ...
          <p
            className="text-xs text-gray-300 truncate mt-1"
      ...
      : []),
    {
      ...
      cell: (b) => (
        <span className="font-bold text-white tabular-nums whitespace-nowrap">
    ...
    >
      ...
      <div className="relative overflow-hidden rounded-3xl border border-white/10 bg-gradient-to-br from-emerald-500/15 via-white/5 to-transparent p-4 sm:p-6">
        ...
        <div className="relative flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between min-w-0">
          ...
          <div className="grid grid-cols-3 gap-2 sm:gap-3 lg:min-w-[380px]">
            ...
            <div className="bg-black/30 border border-white/10 rounded-2xl px-3 py-2.5 text-center min-w-0">
              <p className="text-lg sm:text-xl font-bold text-emerald-300 leading-none truncate">
                ₹{isLoading ? "–" : stats.total.toLocaleString("en-IN")}
              </p>
              <p className="text-[10px] sm:text-[11px] text-gray-400 mt-1 truncate">
                Potential value
              </p>
            ...
            <div className="bg-black/30 border border-white/10 rounded-2xl px-3 py-2.5 text-center min-w-0">
              <p className="text-lg sm:text-xl font-bold text-white leading-none">
                {isLoading ? "–" : `${stats.avg.toFixed(0)} km`}
              </p>
              <p className="text-[10px] sm:text-[11px] text-gray-400 mt-1 truncate">
                Avg trip
              </p>
        ...
        <div className="relative overflow-hidden flex items-start gap-3 bg-gradient-to-br from-amber-500/15 to-amber-500/5 border border-amber-500/30 rounded-3xl p-4 sm:p-5">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-sm font-semibold text-white">
              Active ride · {displayStatus(activeRide.bookingStatus)}
            </p>
            <p className="text-xs text-slate-200/70 mt-1">
              New requests are paused until your current ride completes or is
              cancelled.
            </p>
            <button
        ...
        <div className="flex flex-col gap-2.5 min-w-0">
          ...
          {debouncedSearch && (
            <p className="text-xs text-gray-400" role="status">
              ...
              result{bookings.length === 1 ? "" : "s"} for{" "}
            ...
            >
              {PAY_TABS.map((t) => (
                ...
                >
                  {t.label}
                  <span
                    className={`px-1.5 py-0.5 rounded-md text-[10px] font-bold ${payTab === t.id ? "bg-black/25 text-white" : "bg-white/10 text-gray-400"}`}
                  >
                    {counts[t.id] ?? 0}
                  </span>
                </button>
      ...
      {isLoading ? (
        ...
        <EmptyState
          ...
          title={
            hasActiveRide
              ? "Paused while you ride"
              : debouncedSearch
                ? "No matching rides"
                : payTab === "all"
                  ? emptyTitle
                  : `No ${payTab} rides right now`
          }
          description={
            hasActiveRide
              ? "Finish your current ride to see new requests."
              : debouncedSearch
                ? `Nothing matches “${debouncedSearch}”. Try a place, ID, name, or trip type (one way, round trip).`
                : payTab === "all"
                  ? emptyDescription
                  : "Try another payment filter or refresh the feed."
          }
          action={
        ...
        <div className="grid gap-2.5 sm:gap-3 w-full max-w-full">
          {bookings.map((b, i) => (
            <Motion.article
              key={b._id}
            ...
            >
              ...
              <span
                aria-hidden
                className="pointer-events-none absolute -right-1 -top-2 text-[44px] sm:text-[52px] font-bold text-white/[0.04] leading-none select-none"
              >
                {String(i + 1).padStart(2, "0")}
              </span>

              <div className="relative grid sm:grid-cols-[1fr_176px] min-w-0">
                {/* ── Route side ── */}
                <div className="p-3 sm:p-3.5 min-w-0">
                  <div className="flex flex-wrap items-center gap-1.5 mb-2 min-w-0">
                    {mode !== "mine" && (
                      <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-emerald-500/15 text-emerald-300 border border-emerald-500/25 rounded-full text-[10px] font-semibold">
                    ...
                    <span
                      className="hidden md:inline-flex items-center gap-1 px-2 py-0.5 text-[11px] text-gray-500"
                      title={`Booked ${formatBookedOn(b.createdAt)}`}
                    >
                      <Calendar size={11} /> {formatBookedOn(b.createdAt)}
                    </span>
                  </div>

                  <div className="relative pl-4 space-y-2 min-w-0">
                    <span
                      aria-hidden
                      className="absolute left-[4px] top-1.5 bottom-1.5 w-px bg-gradient-to-b from-green-400/70 via-white/15 to-red-400/70"
                    />
                    <div className="min-w-0 flex items-baseline  gap-2">
                      <span
                        aria-hidden
                        className="absolute left-0 mt-[3px] w-[9px] h-[9px] rounded-full bg-green-400 ring-4 ring-green-400/20"
                      />
                      <p
                    ...
                    <div className="min-w-0 flex items-baseline gap-2">
                      <span
                        aria-hidden
                        className="absolute left-0 mt-[3px] w-[9px] h-[9px] rounded-full bg-red-400 ring-4 ring-red-400/20"
                      />
                      <p
                        className="text-sm font-medium text-white truncate"
                        title={b.drop?.address}
                  ...
                  <div className="flex flex-wrap gap-1.5 lg:mt-12 md:mt-10 mt-4 ">
                    <span className="inline-flex items-center gap-1 px-2 py-1 bg-black/25 border border-white/10 rounded-full text-[11px] text-gray-300">
                      <Navigation size={11} className="text-sky-400 shrink-0" />
                      {b.distance != null
                        ? `${Number(b.distance).toFixed(1)} km`
                        : "—"}
                    </span>
                    <span className="inline-flex items-center gap-1 px-2 py-1 bg-black/25 border border-white/10 rounded-full text-[11px] text-gray-300">
                      <Wallet size={11} className="text-amber-400 shrink-0" />
                      {b.paymentMethod || "Cash"}
                    </span>
                    {b.vehicleType?.name && (
                      <span className="inline-flex items-center gap-1 px-2 py-1 bg-black/25 border border-white/10 rounded-full text-[11px] text-gray-300 max-w-full">
                        <Car size={11} className="text-violet-400 shrink-0" />
                        <span className="truncate">{b.vehicleType.name}</span>
                      </span>
                    )}
                    <span className="inline-flex items-center gap-1 px-2 py-1 text-[11px] text-gray-500">
                      #{b._id?.slice(-6).toUpperCase()}
                    </span>
                  </div>
                </div>
                ...
                <div className="relative flex sm:flex-col items-center sm:items-stretch justify-between gap-2 px-3.5 py-3 sm:p-3.5 bg-gradient-to-br from-emerald-500/15 via-emerald-500/5 to-transparent border-t sm:border-t-0 sm:border-l border-white/10 min-w-0">
                  <div className="min-w-0 sm:text-right">
                    <p className="text-[10px] uppercase tracking-[0.12em] text-gray-400 font-semibold">
                      Est. fare
                    </p>
                    <p className="text-xl sm:text-xl font-bold bg-gradient-to-r from-emerald-300 to-green-400 bg-clip-text text-transparent leading-tight tabular-nums">
                  ...
                  <div className="flex sm:flex-col gap-1 shrink-0">
                    <button
                      ...
                      title="View details"
                      aria-label={`View booking ${b._id?.slice(-6)}`}
                      className="inline-flex items-center justify-center gap-1 p-1 min-w-[40px] min-h-[30px] sm:w-full bg-white/5 border border-white/10 text-gray-200 rounded-xl text-xs font-semibold hover:bg-white/10 active:scale-[0.98] transition-all"
                    >
                      <Eye size={14} />
                    ...
                    {canAct(b) && (
                      <button
                        onClick={() => handleAccept(b._id)}
                        disabled={
                          acceptingId === b._id || rejectMutation.isPending
                        }
                        title="Accept request"
                        aria-label={`Accept booking ${b._id?.slice(-6)}`}
                        className="inline-flex items-center justify-center gap-1 p-1 min-w-[40px] min-h-[30px] sm:w-full bg-gradient-to-r from-green-500 to-emerald-600 text-white rounded-xl text-xs font-semibold hover:shadow-[0_0_25px_rgba(34,197,94,0.5)] hover:brightness-110 active:scale-[0.98] transition-all disabled:opacity-50"
                      >
                        {acceptingId === b._id ? (
                          <Loader2 size={14} className="animate-spin" />
                        ) : (
                    ...
                    {canAct(b) && (
                      <button
                        onClick={() => rejectMutation.mutate(b._id)}
                        disabled={
                          rejectMutation.isPending || acceptingId === b._id
                        }
                        title="Reject request — it stays open for other drivers"
                        aria-label={`Reject request ${b._id?.slice(-6)}`}
                        className="inline-flex items-center justify-center gap-1 p-1 min-w-[40px] min-h-[30px] sm:w-full bg-red-500/15 border border-red-500/25 text-red-300 rounded-xl text-xs font-semibold hover:bg-red-500/25 active:scale-[0.98] transition-all disabled:opacity-50"
                      >
                        <Ban size={14} />

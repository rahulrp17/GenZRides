...
import { Users, Luggage, Snowflake, ArrowRight, CarFront } from "lucide-react";
    ...
    <section className="relative bg-black py-20 md:py-28 overflow-hidden">
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-10">
        ...
        {isLoading ? (
          ...
          <div className="bg-white/5 backdrop-blur-lg rounded-[30px] border border-white/10 p-10 text-center max-w-lg mx-auto">
            <CarFront size={36} className="mx-auto mb-3 text-green-400" />
            <p className="text-white font-semibold">Fleet details are unavailable right now</p>
            <p className="text-gray-400 text-sm mt-1">Please try again in a moment.</p>
            <button
              onClick={() => refetch()}
              className="mt-5 px-6 py-2.5 rounded-full bg-white/5 border border-white/15 text-white text-sm font-semibold hover:border-green-500/50 hover:text-green-300 transition-all"
            >
              Retry
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 md:gap-6">
            {vehicles.map((v, i) => (
              <Reveal key={v._id || v.name} delay={(i % 3) * 0.1}>
                <Motion.div
                  {...cardHover}
